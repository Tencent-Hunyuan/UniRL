...
class HPSv3RewardScorer(LocalRewardBackend):
    ...
    def _compute_model_rewards(self, request: RewardRequest) -> List[float]:
        ...
        for i in range(0, len(images), self.batch_size):
            ...
            with torch.no_grad():
                # Pass by keyword: the hpsv3 package's reward() is ``reward(image_paths, prompts)`` (PyPI 1.0.0) but later source reorders to ``reward(prompts, image_paths)`` — keywords are correct under both.
