...
class LocalRewardBackend(RewardBackend):
    ...
    # Differentiable scoring (ReFL) is an OPTIONAL capability, not part of this base: scorers that wrap a differentiable nn.Module define ``compute_rewards_differentiable`` and thereby satisfy the ``unirl.reward.base.DifferentiableReward`` Protocol (e.g. PickScore).
