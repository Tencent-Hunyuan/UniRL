...
class RewardRequest(Batch):
    ...
    @classmethod
    def concat(cls, items: Sequence["RewardRequest"]) -> "RewardRequest":
        """Concatenate requests only when their modality maps have identical schemas."""
        if items:
            for field_name in ("primitives", "generated"):
                expected = set(getattr(items[0], field_name))
                for index, item in enumerate(items[1:], start=1):
                    actual = set(getattr(item, field_name))
                    if actual != expected:
                        raise ValueError(
                            f"RewardRequest.concat requires identical {field_name} keys; "
                            f"item 0 has {sorted(expected)}, item {index} has {sorted(actual)}."
                        )
