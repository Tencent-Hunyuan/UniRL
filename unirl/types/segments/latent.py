...
class LatentSegment(Segment):
    ...
    # Start-of-denoising latent (x_T or encoded init image), per-sample, set on the gen-shell before generation (img2img / driver-materialized x_T).
    ...
    # Optional auxiliary per-step latent trajectory stored at the SAME sparse ``indices`` as ``latents`` (shape ``[B, K, ...]``). First consumer: LTX-2, which co-denoises an AUDIO latent stream alongside the video one — the video transformer forward depends on the current audio state (audio→video cross-attention), so replay must reproduce the audio at each step.
