...
class AllSDEScheduler(TimestepScheduler):
    ...
    def get_sde_indices(self, step: Optional[int] = None) -> Set[int]:
        # ``num_sde_steps=0`` is the forward-process / DiffusionNFT path: no step runs SDE, no log_prob is captured. The rollout driver reads ``None`` / empty set the same way.
