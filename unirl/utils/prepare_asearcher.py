...
def _iter_source(source: Optional[str], hf_name: str, split: str) -> Iterator[Dict[str, Any]]:
    ...
    # streaming=True yields raw rows and skips the arrow schema cast that fails when building the full DatasetDict (the sibling ASearcherLRM35k split has an inconsistent schema), and avoids downloading the whole set up front.
...
def main() -> None:
    ...
    # ASearcherBase35k loads cleanly; ASearcherLRM35k (AReaL's exact split) has an inconsistent HF schema (extra id/idx cols, scalar answer) that `datasets` can't auto-cast — download its raw jsonl and pass it via --source to use it.
