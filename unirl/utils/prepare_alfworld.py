...
# Non-empty so the data source keeps the row; AlfworldEnv.reset() replaces it with the environment's initial observation, so the text itself is never used for generation.
...
def main() -> None:
    ...
    if args.limit and args.limit < len(games):
        # Evenly spaced across the sorted games so a small fixed set spans task types (sorted games cluster by type). A fixed set that a rollout FULLY covers (prompts_per_rollout == #games) makes the per-rollout reward comparable across rollouts — the curve then reflects learning, not which games were drawn.
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        for i in range(len(games)):
            # Carry the exact game FILE so the env plays precisely this row's game (index alone drifts once the list is filtered/sampled).
