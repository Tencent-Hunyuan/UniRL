...
def _unpack_qwen_moe(model: torch.nn.Module) -> bool:
    ...
    # VeOmni train-side EP keeps only this rank's contiguous [E/ep, ...] expert block in the model. FullWeightSync._iter_full_tensors_ep must see the packed parameter name so it can gather those blocks across the EP group and assign global expert ids before exporting HF per-expert keys.
...
def merged_state_dict(
    ...
    for original_name, group in lora_groups.items():
        ...
        if skip_lm_head and original_name == "lm_head.weight":
            continue
        # Merge inputs stay master-width (no ``dtype`` here): pre-casting them to the wire dtype would round the LoRA update away before the fold.
        if (
            unpack_qwen_moe
...
def extract_lora_tensors(
    ...
    *,
    ...
    # Defensive dtype check: vllm punica kernel hard-asserts inputs.dtype in {fp16, bf16}. Catch fp32 LoRA here in trainer (cheap) rather than crashing ~20min later in rollout.
    _bad_dtype = [
