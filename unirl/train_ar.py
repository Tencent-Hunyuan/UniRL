...
def main(cfg: DictConfig) -> None:
    ...
    def teardown() -> None:
        # Still None if ARTrainer.__init__ raised — an engine that booted before the failure has no handle to reach here, so it cleans up after itself inside VLLMOmniBackend.boot rather than relying on this path.
    ...
    with GracefulShutdown(teardown, name="ar-train") as guard:
        ...
        # Backstop for the ray.init hook: if Ray was auto-initialised off the main thread, that hook could not install handlers and this is the first point where we are back in the main thread with Ray up.
        guard.claim_signals()
        trainer.train(
