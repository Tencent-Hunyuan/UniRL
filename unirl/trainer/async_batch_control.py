...
class AsyncBatchControl:
    ...
    def sync_rollout(self, manager: Any, weight_sync: Any, *, force: bool = False) -> None:
        if not force and self.publish_lag == 0:
            self.batches_since_sync = 0
            return
        carried = manager.quiesce(current_version=self.train_version)
        if carried:
            raise RuntimeError(f"batch rollout quiesce returned {len(carried)} undispatched tasks")
        manager.sync_weights(weight_sync, output_version=self.train_version)
