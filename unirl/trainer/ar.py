...
# Generous enough for a healthy engine — a TP group tearing down NCCL and its CUDA contexts takes tens of seconds — while still bounded well under the driver's own hard deadline so the pool teardown that follows gets to run.
...
class ARTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        *,
        ...
        # True (default) = standard GRPO: divide the group-relative advantage by the group std. False = mean-center only (reward - group_mean), NO std division — removes the difficulty bias that over-amplifies low-std (hard) prompts.
        ...
        # verl trainer.balance_batch parity: driver-side reorder of the rollout batch so each DP shard receives a similar total-token workload. FSDP collectives sync all ranks every micro, so a step runs at the SLOWEST rank's pace — without balancing, the rank that drew the longest sequences straggles (~+/-11%% rank-total variance at heavy lengths).
        ...
        with placement(self.pool, fraction=1.0, shared_workers=True):
            ...
            if self._rollout_anchor_device is None:
                # Default SPMD rollout path. For a separate colocated rollout engine, release FSDP before boot so SGLang sizes its fixed pools against rollout-phase capacity rather than temporarily resident training state.
                ...
                try:
                    ...
                    # Match the colocated lifecycle used by verl: keep the inference engine asleep while the training state owns the GPU. A trainside rollout has no sync role and shares the live training model, so it must stay awake.
                    ...
                    if bootstrap_offloaded and not rollout_boot_started:
                        ...
                        # The rollout is already safely asleep; preserve the original error (for example, a failed FSDP onload) without retrying it.
                        pass
                    raise
            else:
                ...
                if self._enable_fsdp_offload:
                    ...
                    # Resident colocate: enable_fsdp_offload=false keeps the training FSDP shards on-GPU, so keep the rollout engine awake alongside them (both resident, no per-step swap). vLLM's low gpu_memory_utilization leaves room for both.
    ...
    def _prepare_rollout(self, *, sync_weights: bool) -> bool:
        ...
        try:
            if do_sync and do_offload and self._supports_staged_wake:
                # Keep the large KV and CUDA-graph regions paused during the full FSDP gather. A following full wake resumes only those remaining regions because SGLangRolloutEngine tracks the weights stage.
                self.rollout.wake_up(tags=["weights"])
                self.weight_sync.sync()
                train_state_maybe_offloaded = True
                self.backend.offload()
                # If this call fails after SGLang resumed only some tags (or the response is lost), its lifecycle flags cannot prove which regions are live. Fail closed with FSDP left on CPU rather than risk a double-resident onload.
                ...
                self.weight_sync.sync()
                if do_offload:
                    # Non-SGLang backends have no tagged wake, so their full wake and sync must precede the training-state handoff.
                    train_state_maybe_offloaded = True
                    self.backend.offload()
            elif do_offload:
                # No sync needs the train model this round, so release it before restoring any SGLang/vLLM region.
                train_state_maybe_offloaded = True
                self.backend.offload()
                full_wake_after_train_offload_in_progress = True
                self.rollout.wake_up()
            ...
            # Recover when the ownership state is known. During a failed full wake after offload, keep the training state parked as above.
    ...
    def _finish_rollout(self, *, train_state_offloaded: bool) -> None:
        ...
        # If sleep fails, do not put the training state back on the same GPUs: the inference regions may still be live and an onload would turn the original lifecycle error into a process-killing OOM.
        self.rollout.sleep()
    ...
    def train_step(
        ...
        *,
        ...
        if not anchored:
            # SPMD path: rollout sibling of every train rank; sync with train model live, optionally hand GPU ownership to rollout for generate, then sleep rollout before restoring train state.
        ...
        # verl balance_batch parity: reorder so each DP shard gets a near-equal token load before DP_SCATTER (no-op when already balanced).
        ...
        try:
            ...
            # Match UnifiedModelTrainer's steady state: FSDP on CPU and the rollout asleep between steps. This also reclaims every rank's eager-load/activation allocator cache, not only the anchor rank.
            if anchored:
                self._ensure_anchored_backend_offloaded()
    ...
    def evaluate(self, rollout_id: int) -> float:
        ...
        # Anchored eval keeps FSDP offloaded and vLLM awake for the entire eval set. Training still uses one _anchored_rollout_session per rollout in train_step(), so its sleep/wake and onload/offload lifecycle is unchanged.
        eval_session = (
        ...
        # MC reward is 0/1 so mean reward == accuracy; also emit it as `reward` so this run shares the eval/reward axis with the other trainers.
    ...
    def _shutdown_runtime(self) -> None:
        ...
        # train()'s finally and the entrypoint's teardown both land here; the second pass must not re-enter shutdown on already-dead Ray actors.
        ...
        if callable(shutdown):
            # Bounded: this is a blocking call into a single-threaded Ray actor, so it queues behind whatever that actor is still doing. An engine wedged mid-generate would otherwise keep us out of pool.shutdown() indefinitely — and that is the step that frees the GPUs.
