...
def build_optimizer(
    ...
    *,
    ...
    # ``foreach=False`` disables the multi-tensor kernel path. Required whenever the param list mixes regular ``torch.Tensor`` (non-FSDP-wrapped sub-modules — e.g. SD3's embed/norm layers when only transformer blocks are ``fully_shard``-wrapped) with ``DTensor`` (FSDP-wrapped block params): ``_foreach_lerp_`` rejects the mixed bag and trips ``RuntimeError: got mixed torch.Tensor and DTensor``.
    adam_kwargs = dict(
