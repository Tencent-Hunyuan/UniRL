...
class UnifiedModelTrainStack(Remote):
    ...
    def __init__(
        ...
        *,
        ...
        # PPO-style multi-update: split each rollout shard into this many disjoint mini-batches and run ONE optimizer step per mini-batch, with the π_old anchor frozen once across all of them (prepare_segment). >1 makes the clip / ratio trust region actually engage (the 2nd+ step is off-policy); 1 (default) keeps the prior single-step behavior. BOTH algorithms must keep their anchor frozen across the N steps (supports_multi_update).
    ...
    def _train_one_step(
        ...
        *,
        ...
        if any_backward:
            # Multi-update only: the prior update's forward/backward churn fragments the CUDA pool, so this step's clip_grad_norm NCCL all_reduce can fail to find a contiguous buffer (OOM with free-but-fragmented memory — exactly the num_updates_per_batch>1 optimizer-step OOM). Gated on >1 so the single-update path (and the LoRA recipe) pays nothing.
    ...
    def train_track(
        ...
        *,
        ...
        # Move both tracks onto this worker's model device before any replay. The HI3 rollout tracks are hydrated to CPU on the driver (the two anchored engines return single transport handles that the driver materializes off-GPU before re-sharding), so segment latents / AR tokens / fused conditions arrive on CPU while the backbone is on cuda.
        device = self.fsdp_backend._device
        ar_part = ar_part.to_device(device)
        image_part = image_part.to_device(device)

        # Only UNIRL_PROFILE=train applies here (one-update lives in TrainStack._run_updates); warn if one-update was set so it isn't silently ignored.
        ...
        with profiler.record("train_track") if profiler is not None else nullcontext():
            ...
            # N optimizer steps over disjoint mini-batches (each Part sliced by the same shared _optimizer_step_slices; M=1 keeps ar/image 1:1 and equally sized).
        ...
        # Reduce each track's per-optimizer-step results into one summary, attaching each optimizer step's own metrics on ``per_update`` so the logger emits ONE wandb point per optimizer update (on-policy update0 vs off-policy update1+ stay distinct series instead of being averaged into one misleading ratio_mean).
