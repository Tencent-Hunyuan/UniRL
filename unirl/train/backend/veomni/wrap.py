...
def veomni_parallelize(
    ...
    *,
    ...
    # Master-weight dtype: cast on meta (dtype-only, no data) so to_empty materializes storage in this dtype; MixedPrecisionPolicy(param_dtype) then casts the compute copy to bf16. master_dtype=None -> master follows param_dtype (all-bf16).
    master_t = (
    ...
    # Expert-parallel bookkeeping: VeOmni sets ``model._extra_parallel_param_groups`` inside *its* optimizer builder (veomni/optim/optimizer.py), which UniRL does not use (we build our own optimizer). The EP-aware grad clip (veomni.distributed.fsdp2.clip_grad_norm) dispatches on this attribute — so without it, an EP-sharded model falls back to the non-EP clip and crashes trying to stack grads across the ep_fsdp (size world/ep) and dp_shard (size world) meshes.
...
def _enumerate_block_instances(
    ...
    # ``parallelize_model_fsdp2`` (run before this) ``fully_shard``s each block, which renames its class to ``FSDP<OriginalName>``; strip that prefix before matching (a no-op when absent) so AC actually finds the post-shard blocks.
