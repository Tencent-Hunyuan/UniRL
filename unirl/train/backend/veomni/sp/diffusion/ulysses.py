...
def _make_ulysses_dispatch(orig_dispatch: Callable) -> Callable:
    ...
    def ulysses_dispatch(query: Tensor, key: Tensor, value: Tensor, *args: Any, **kwargs: Any):
        ...
        # Intervene only for SP self-attention. Cross-attention (e.g. Wan text branch: sliced image Q vs full text K/V => unequal seq lengths) attends locally per rank and also passes through -- the image Q is already sliced to local by the boundary hook, so it must NOT be all-to-all'd.
        ...
        # Self-attention joint all-to-all: each rank gathers the full sequence and keeps its slice of heads, attends over the full (order-invariant) set, then inverts.
        g = ps.sp_group
...
# Per-model boundary hooks: slice streams + RoPE in, gather hidden out. Three models use the BLOCK-level pattern (slice at blocks[0], gather at norm_out) via _install_boundary_hooks; flux2 uses a MODEL-level pattern (slice both streams at model input, gather image at model output) because its dual->single + text-strip layout has no single block boundary -- kept inline in _wrap_flux2.
...
def _install_boundary_hooks(
    ...
    # block0_pre records the pre-slice image length; norm_out_pre reads it to drop SP divisibility padding after the gather.
    state: Dict[str, int] = {}
    import torch.distributed as dist
...
def apply_diffusion_sequence_parallelism(model: nn.Module, sp_size: int) -> None:
    ...
    # Walk the MRO, not just type(model).__name__: after veomni_parallelize the instance's class is a dynamic FSDP2 subclass (e.g. FSDPSD3Transformer2DModel) whose base is the real transformer class, so the bare-name registry would miss.
