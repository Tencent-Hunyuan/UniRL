...
class FSDPBackend(BaseFSDP2Backend):
    ...
    def __init__(
        ...
        *,
        ...
        # FSDP compute dtype (MixedPrecisionPolicy.param_dtype) = the wire dtype for weight sync. With master_dtype=fp32 the trainable LoRA params live in fp32 (the reward-collapse fix), but the rollout engine's vLLM punica kernel hard-asserts bf16/fp16 — so LoRA extraction casts to this dtype at the all-gather (also halves sync bandwidth).
        ...
        # Real weights: meta-init bundles stash a safetensors dir (load_sharded to_empty-materializes the still-meta module then broadcasts); Pattern-A bundles (hi3) materialize themselves; eager bundles already hold real weights (fsdp_wrap sharded them in place), so eager_ok=True is a no-op.
    ...
    def _clip_grad_norm(self, max_grad_norm: float) -> torch.Tensor:
        # Every trainable grad is a sharded DTensor that FSDP reduce-scatters: the root wrap claims the leftover params, and fsdp_wrap fails fast on trainable params outside every group when root_wrap is disabled.
