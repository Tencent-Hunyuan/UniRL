...
def _extract_canonical_lora(backend: Any, *, param_prefix: str, adapter_name: str):
    ...
    The wire dtype is the backend's FSDP compute dtype (``backend.wire_dtype``,
    i.e. ``param_dtype``), NOT the trainable params' own dtype: under
    ``master_dtype=fp32`` (the reward-collapse fix) the LoRA params are fp32, but
    the rollout engine's vLLM punica kernel requires bf16/fp16 — so cast at the
    all-gather. Falls back to ``None`` (keep dtype) for backends predating
    ``wire_dtype`` (e.g. an all-bf16-master setup where no cast is needed).
    """
