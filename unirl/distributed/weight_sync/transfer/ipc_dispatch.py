...
UNIRL_LORA_INT_ID: int = 1
UNIRL_LORA_NAME: str = "unirl_lora"
UNIRL_LORA_PATH: str = "unirl_lora_in_memory"

_DEFAULT_IPC_DIR: str = os.environ.get("UNIRL_IPC_DIR", "/tmp")
...
def zmq_handle(replica_rank: int, stage_id: int, local_rank: int, *, ipc_dir: str | None = None) -> str:
    ...
    return f"ipc://{root}/unirl-zmq-replica-{int(replica_rank)}-stage-{int(stage_id)}-rank-{int(local_rank)}.sock"


def replica_rank_from_env() -> int:
    """Read the rollout-actor replica rank from env (default 0)."""
    return int(os.environ.get("UNIRL_REPLICA_RANK", "0"))
