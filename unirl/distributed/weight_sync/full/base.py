...
class FullWeightSync(Remote):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        # Deferred: unirl.utils.dtypes imports torch at module scope, and this module must stay driver-importable without torch.
        ...
        # Which PEFT adapter's weights to push. See class docstring; a non-default adapter must be merged in, so fail closed.
        ...
        # The pushed adapter is invisible in the recipe when it defaults from the backend (DiffusionNFT resolves to the EMA shadow "old"), so log it once: a SEPARATE rollout engine sampling under "old" is the intended faithful path, not a misconfiguration.
        ...
        # Ordered, first-match-wins name rewrites (glob -> replacement, or None to drop); see _validate_name_remap / _apply_name_remap for the contract.
    ...
    def _iter_full_tensors(self) -> Iterator[Tuple[str, "object"]]:
        ...
        # Expert-parallel models always need the EP-aware walk: even when LoRA is merged, the frozen fused experts still carry only this rank's [E/ep] block and must be gathered + converted to HF per-expert names.
