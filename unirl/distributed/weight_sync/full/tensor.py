...
class TensorWeightSync(FullWeightSync):
    ...
    def sync(self) -> None:
        ...
        # Use SGLang's own reductions when the rollout engine is SGLang-based so pickles reference ``sglang.srt.utils.patch_torch._rebuild_cuda_tensor_modified`` — the server-side ``SafeUnpickler`` allows ``sglang.srt.utils.`` but NOT ``unirl.``, so the vendored copy in ``sgl_compat`` only works for vLLM-Omni (where the receiver is a vLLM worker, not SGLang's SafeUnpickler). When both sglang and vllm are installed, detect the engine kind from the rollout sibling so vLLM-Omni doesn't accidentally use SGLang's reductions.
        ...
        for bucket, is_last in self._iter_buckets():
            # Group by dtype, one FlattenedTensorBucket per dtype (matches the receiver's flattened_bucket load_format). Non-tp-zero SGLang TP ranks must also serialize their LOCAL CUDA IPC handles; tp_rank=0 forwards those small serialized handles to the hosted SRT server.
            by_dtype: dict = {}
            ...
            # Non-tp-zero ranks still drive the generator (lockstep all-gather). For SGLang TP they additionally serialize a local-device payload and participate in the handle gather; otherwise their rollout is a no-op shell and there is nothing to push.
            ...
            for i, grouped in enumerate(groups):
                ...
                # SGLang TP payloads exported by non-tp-zero ranks must stay alive until tp_rank=0's blocking HTTP/native update returns. This status gather is both a barrier and error propagation.
            ...
            # Release the all-gathered full tensors + IPC payloads for this bucket before gathering the next — else the full model (~13GB) accumulates in the caching allocator and OOMs the colocated server.
            del groups, by_dtype, bucket
            if torch.cuda.is_available():
                # CUDA IPC exports enter the allocator's limbo after the receiver closes them. empty_cache() alone cannot reclaim those segments; ipc_collect() completes the refcount handshake on the rank that created the exports.
