...
class SGLangRolloutEngine(SyncRolloutEngine):
    ...
    # Marks this role as accepting per-rank rollout-TP kwargs (tp_rank/tp_size/ tp_visible_devices/pp_rank/ep_rank/ep_size) from Handle. Read by ``_is_sglang_rollout_role`` in group/handle.py to gate the kwargs injection — a string-name check would silently break on rename or subclassing, so an explicit opt-in flag is safer.
    ...
    def __init__(
        ...
        *,
        ...
        # LLM engine carries its own model path on the config; the diffusion engine takes it from model_config. Log if a caller supplied one so the divergence is visible.
        ...
        # Rollout tensor-parallel layout. Handle injects these per-rank when the rollout Handle carries tp/pp/ep > 1. tp_rank==0 boots a (possibly multi-GPU) SGLang engine spanning ``tp_visible_devices``; every other TP rank is a no-op shell that still occupies its Worker for training but holds no SGLang server (SGLang's own scheduler subprocesses own those GPUs). tp_size=1 (the default) reproduces the one-engine-per-GPU path.
        self._tp_rank = int(tp_rank)
        self._tp_size = int(tp_size)
        self._pp_rank = int(pp_rank)
        self._pp_size = int(pp_size)
        # ep_rank/ep_size are recorded for completeness / future use: EP is sharded INSIDE SGLang (within the TP group), so UniRL never branches on them here.
        ...
        if tp_visible_devices is not None:
            ...
            # Direct, non-Handle callers historically passed node-local integer ids. Preserve that narrow API while preventing Handle from ever deriving them from cluster-global ids.
        ...
        if not self._is_tp_zero:
            # No-op shell: no SGLang server, no ports, no weight sync. The adapter is still built so any pure-Python helpers stay available, but nothing touches the GPU here.
            self.adapter = None
            self._backend = None
        ...
        # Tokenizer (+ AutoProcessor for VLM) — the encoding I/O the engine owns, injected into the adapter so its conversion methods stay pure. The processor encodes multimodal prompts the SAME way the trainside replay does (it expands the single image placeholder and emits pixel_values / image_grid_thw), keeping rollout and replay token-for-token aligned.
        ...
        # Ports — engine-reserved on this node at the last moment before the spawn (both backends: nccl_port de-syncs colocated engines).
        if ports is None:
            ports = SGLangPorts.reserve()

        # Rollout-TP runtime overrides. The backend applies the Worker's actual Ray CUDA tokens only at the scheduler spawn boundary and resets base_gpu_id to zero inside that restricted logical device list.
        ...
        if config.backend == "native":
            ...
            # The address peers reach this server at (the bind host is usually the 0.0.0.0 wildcard). Node-identity discovery, not runtime I/O — and HTTP-only: it exists to build the client base_url.
        ...
        # The backend owns its runtime concurrency. The engine only owns policy provenance and delegates generation through the seam.
    ...
    # Lifecycle — the offload flags live here; decorators re-applied (synchronous.py footgun)
    ...
    def sleep(self, tags: Optional[List[str]] = None) -> None:
        ...
        self._weights_onloaded_for_sync = False
        # Releasing weights frees the SRT LoRA pool; the adapter must be re-pushed (set_lora_from_tensors) before it can be referenced again.
    ...
    # Weight sync — frozen synchronous.py surface; thin forwards to the component. Un-decorated: reached per worker via the raw ``Worker.call`` RPC, not through ``@distributed``.
    ...
    # ``update_weights_from_ipc`` is deliberately NOT defined — the base raises NotImplementedError (SGLang has no bucketed-IPC receiver).
