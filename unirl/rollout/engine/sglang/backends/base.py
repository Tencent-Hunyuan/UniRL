...
def _preloaded_cuda_driver_dirs() -> List[str]:
    ...
    # torch-memory-saver replaces LD_PRELOAD while starting SGLang schedulers;
    # retain the shim directory in LD_LIBRARY_PATH so children resolve libcuda.
...
def _preserve_cuda_driver_preloads(
    ...
    # LD_LIBRARY_PATH cannot override an executable RPATH to the host driver.
    # Preload the compatibility driver first and retain it for descendants.
...
def _configure_torch_memory_saver_composite_preload() -> None:
    ...
    # Hook mode treats the full environment value as one ctypes path. Select
    # only its hook while preserving the compatibility driver for descendants.
    try:
...
def _scheduler_spawn_environment(
    ...
    # Ray workers can inherit a stale LD_LIBRARY_PATH from the daemon. Prepend
    # active wheel libraries for the child, then restore the worker environment.
