...
def _resolve_param_names_mapping(module) -> dict:
    ...
    SGLang models that FUSE projections (e.g. Z-Image: ``to_q/to_k/to_v`` →
    ``to_qkv``, ``feed_forward.w1/w3`` → ``w13``) carry a class-level
    ``param_names_mapping`` of ``{regex: (replacement, shard_id, num_shards)}``.
    The initial checkpoint load applies it; the in-memory update path does not,
    so without this the separate-projection weights silently never land (the
    bug behind a flat reward curve on fused-attention models like Z-Image).
    """
...
def patch_weights_updater() -> None:
    ...
    if not getattr(wu.load_weights_into_model, _LWIM_SENTINEL, False):
        ...
        def load_weights_into_model(weights_iter, model_params: dict) -> None:
            ...
            # Silent weight-drop is dangerous: a name that matches no model param
            # is skipped above (``continue``), so a sender/receiver naming or
            # fusion mismatch (e.g. diffusers separate ``to_q/to_k/to_v`` vs
            # SGLang's fused ``to_qkv``) leaves those weights at their loaded base
            # value with NO error — the rollout silently never sees the trained
            # update. Surface it loudly with a count + sample.
            if _skipped:
                logger.warning(
    ...
    def _apply_named_tensor_weights(
        ...
        for module_name, module in modules_to_update:
            ...
            try:
                ...
                # applied so separate-projection tensors land in the right fused
                # shard. Consume those here; the rest fall through to the
                # exact-match loader below.
