...
class FastVideoRolloutEngine(SyncRolloutEngine):
    ...
    def __init__(
        ...
        *,
        ...
        # Last checkpoint pushed by the weight sync. ``VideoGenerator`` loads the PRETRAINED weights from ``model_path`` on every (re)build, so a sleep/wake would silently roll back to pretrained; we re-apply this on wake.
        ...
        # σ SSOT: same schedule policy the trainer/replay uses, so the engine can pin the frontier's diffusion.sigmas and FastVideo consumes it exactly.
        ...
        # ``VideoGenerator`` is synchronous and not request-concurrent-safe. Match the other dedicated single-turn engines: serialize generation, make shutdown idempotent, and stamp every filled frontier with the successfully loaded policy-weight version.
    ...
    def _build_generator(self) -> None:
        ...
        fv_kwargs: Dict[str, Any] = {
            ...
            "inference_mode": True,
            # Force decoded pixels as a [B, C, T, H, W] tensor (not PIL/latent) so execute_forward populates batch.output for the reward path.
            "output_type": "pt",
        ...
        # WanT2V480PConfig (1.3B) defaults flow_shift=3.0. FastVideo re-applies flow_shift inside set_timesteps even for custom sigmas, so pipeline_config.flow_shift MUST match model_config.shift or native old_logp and trainer replay diverge.
    ...
    def _generate_core(self, sample: Sample) -> Sample:
        ...
        # σ SSOT: pin once on the frontier's shared sampling_params. Frontier slices preserve that shared object, so every chunk sees one schedule.
        ensure_sample_sigmas(sample, self.schedule_policy)

        # ``forward_batch_size`` here is a CHUNKING cadence, NOT a GPU batch size: ``_drive_fastvideo`` runs FastVideo one video at a time (per-sample seeds preclude a batched forward), so peak GPU activation is fixed at one video regardless of ``fbs``. What ``fbs`` bounds is how many per-sample outputs (trajectory/decoded tensors, already on CPU) accumulate before a concat + ``empty_cache``.
        fbs = self.cfg.forward_batch_size
        bs = int(gen.batch_size)
        if fbs is None or bs <= fbs:
            return self._generate_batch(sample)

        # Slice only the frontier. The ancestor input Part(s) stay whole so ``Sample.text_conditioning`` can project them onto each chunk's ids.
    ...
    def _drive_fastvideo(
        ...
        # σ SSOT — AVOID the double-shift bug. So hand FastVideo the shift PRE-IMAGE g, for which FastVideo's own shift reproduces diffusion.sigmas exactly: g = s / (shift - s·(shift-1)) ⇒ shift·g/(1+(shift-1)·g) == s (valid because FastVideo's WAN flow_shift == model_config.shift).
        ...
        # SDE window handed to FastVideo's denoiser so it injects exploration noise ONLY on the trainer's SDE steps and runs the rest as a deterministic Euler step (clean low-sigma tail). ``None`` keeps the legacy all-steps fallback; an explicit empty list means no SDE steps.
        ...
        for prompt, seed in zip(prompts, seeds):
            ...
            # Text conditioning: reuse the *exact* prompt embeddings FastVideo fed its transformer this rollout, so the trainer's replay forward yields an on-policy importance ratio (no re-encode drift). prompt_embeds is a per-encoder list; WAN uses a single UMT5 encoder -> index 0.
            ...
            # Per-video: outputs are already copied to CPU above, so drop this video's GPU tensors before the next iteration. This — not ``forward_batch_size`` — is what actually bounds peak GPU memory, since the forward runs one video at a time.
            del out, rl, traj, dec
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
    ...
    def _build_segment(self, params: DiffusionSamplingParams, raw: Dict[str, Any]):
        ...
        # Mirror the SGLang reference: ``None`` means every transition is an SDE step, while an explicit empty list is a deterministic forward process and leaves ``sde_indices`` absent.
        ...
        # sde_logp: native per-step log-prob [B, T] from FastVideo's RLData. Slice to the SDE columns when a strict subset was requested; otherwise the full [B, T] already matches the all-steps schedule.
        sde_logp = None
    ...
    def _wake_up_locked(self) -> None:
        ...
        # MultiprocExecutor's TCPStore must bind a master port every time the generator is rebuilt. Refresh the hint for every attempt and self-heal the rare EADDRINUSE race instead of killing the Ray actor/run.
        max_port_attempts = 5
        for attempt in range(1, max_port_attempts + 1):
            self._ports = FastVideoPorts.reserve()
            self._fastvideo_args.master_port = int(self._ports.master_port)
        ...
        # ``from_fastvideo_args`` reloads the PRETRAINED transformer from ``model_path``; without this the engine would sample under pretrained weights on every wake that isn't immediately followed by a weight sync (i.e. any ``weight_sync_interval > 1`` step). Re-apply the last synced checkpoint so wake is weight-preserving, matching the other engines' sleep/wake contract (sglang resume_memory keeps weights resident).
        try:
            ...
            # Fail closed: a rebuilt generator contains pretrained weights until the cached checkpoint is restored. Keep the engine offloaded so a retry cannot silently skip restoration and serve stale weights.
            try:
                if self._generator is not None:
                    self._generator.shutdown()
    ...
    def shutdown(self) -> None:
        with self._shutdown_lock:
            if self._shutdown_complete:
                return
            # First wait for an active generation and close admission. A caller already queued on ``_generate_lock`` will then fail the shutdown guard rather than entering FastVideo during teardown.
            with self._generate_lock:
    ...
    # Weight sync — checkpoint_path (full-param hot-swap). Reached per worker via the local sibling call from CheckpointWeightSync (not @distributed).
    def update_weights_from_path(self, checkpoint_path: str, *, track_prefix: str = "") -> None:
        ...
        with self._generate_lock:
            ...
            # Only a successful load changes either provenance field. Remember the path so ``wake_up`` can re-apply after a rebuild, then bump the version stamped onto subsequently generated Parts.
