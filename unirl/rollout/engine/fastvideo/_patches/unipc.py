"""Runtime patch routing FastVideo deterministic transitions through UniPC.

FastVideo's WAN pipeline owns a FlowUniPC scheduler, but its RL branch bypasses
``scheduler.step`` and hard-codes Euler on non-SDE indices. This adapter keeps
FastVideo as the model executor while routing those deterministic transitions
through :class:`unirl.sde.unipc.UniPCStrategy`.
"""
...
_PLAN_CONTEXT: ContextVar["FastVideoUniPCPlan | None"] = ContextVar(
    "unirl_fastvideo_unipc_plan",
    default=None,
)
...
def _patch_scheduler_set_timesteps() -> None:
    ...
    def set_timesteps(
        ...
        if np.any(external[1:] >= external[:-1]):
            raise ValueError("FastVideo canonical sigmas must be strictly decreasing")
        if external[-1] == 0.0:
...
def _patch_denoising_step() -> None:
    ...
    def sde_step_with_logprob(
        ...
        if not isinstance(plan, FastVideoUniPCPlan):
            return original(
                scheduler,
                model_output,
                timestep,
        ...
        if step_index in plan.sde_indices:
...
def patch_unipc() -> None:
    """Install canonical schedule and SDE/UniPC dispatch patches."""
