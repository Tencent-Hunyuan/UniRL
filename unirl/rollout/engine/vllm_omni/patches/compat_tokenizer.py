...
# Module-import side effect: fires when vllm-omni's worker subprocess imports this module to resolve the ``HI3ARWorkerExtension`` qualname. Idempotent via the ``_INSTALLED`` guard, so re-imports are safe.
install()

# Side-effect: install the HI3-specific MoE-LoRA compat patch needed when ``enable_lora=true`` on the AR stage. Importing the module fires its ``install()``; the patch is idempotent and a no-op if vllm isn't loaded.
