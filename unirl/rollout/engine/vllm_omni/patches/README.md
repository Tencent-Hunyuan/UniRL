...
| `patch_hv15_refiner_qkv_weight_loader` | Pinned vllm-omni treats every HV1.5 `.to_q`/`.to_k`/`.to_v` checkpoint key as a packed main-block projection. The token refiner keeps those projections as ordinary `nn.Linear` layers, so the stock loader takes its packed-name branch, finds no packed parameter, and silently leaves all 12 refiner Q/K/V weight and bias tensors at random initialization. Stream those unfused tensors through the ordinary loader while retaining packed loading for the 54 main blocks | vllm-omni loads an existing unfused parameter directly when its synthesized packed parameter does not exist |
| `patch_hv15_packed_lora_mapping` | HV1.5 declares its packed QKV relationship only as a local variable inside `load_weights`, while the pinned diffusion LoRA manager searches a model-level `stacked_params_mapping`. Consequently all six logical Q/K/V targets per main block are loaded but neither packed projection is wrapped or activated. Expose the loader's existing mapping at model scope | vllm-omni's HV1.5 model exposes a packed LoRA mapping that its manager consumes, or the manager reads `packed_modules_mapping` directly |
...
| `patch_hv15_autocast_forward` | Trainside wraps every HV1.5 transformer call in BF16 autocast, while pinned vllm-omni calls the BF16 model without autocast. The different CUDA op policies perturb the rollout/replay ratio, so enter autocast at the vLLM transformer boundary as well | vllm-omni runs the HV1.5 transformer under the same autocast policy as Diffusers replay, or rollout and replay share one transformer implementation |
| `patch_hv15_qk_rmsnorm` | Pinned vllm-omni forces its fused `vllm_c` RMSNorm kernel for HV1.5 Q/K, while Diffusers replay uses an fp32 variance/normalization followed by a cast back to the weight dtype. The two paths differ on BF16 inputs and perturb rollout/replay log-prob ratios across the recipe's `1e-4` clip threshold. Keep vLLM's parameters and weight loading but use the Diffusers arithmetic ordering for the four Q/K norms in each HV1.5 attention block | vllm-omni's HV1.5 Q/K normalization matches Diffusers, or rollout and replay share one transformer implementation |
| `patch_hv15_sdpa_attention_mask` | Pinned vllm-omni drops an all-true SDPA mask, while Diffusers materializes and passes its symmetric boolean mask. PyTorch selects a numerically different BF16 SDPA path when the mask is omitted, so rebuild the exact Diffusers mask on each HV1.5 SDPA instance | vllm-omni preserves HV1.5's all-true boolean mask, or rollout and replay share one transformer implementation |
| `patch_hv15_rotary_embedding` | Pinned vllm-omni casts HV1.5 RoPE frequencies to BF16 before its fused rotary kernel, while Diffusers multiplies BF16 Q/K by fp32 frequencies and casts only the result. Rebuild Diffusers' repeated real-frequency layout and arithmetic while leaving the rest of vLLM's attention forward unchanged; self-skips if the pinned forward signature changes | vllm-omni preserves fp32 HV1.5 RoPE frequencies through the multiply, or rollout and replay share one transformer implementation |
