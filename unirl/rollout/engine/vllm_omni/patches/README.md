...
| `patch_hv15_sdpa_attention_mask` | Pinned vllm-omni reduces HV1.5's padded text mask to broadcast `[B, 1, 1, K]`, while Diffusers replay uses a symmetric `[B, 1, Q, K]` mask. The logically equivalent layouts can select different SDPA kernels and move rollout/replay log-prob ratios across the recipe's `1e-4` clip threshold. Expand only the HV1.5, SDPA, non-SP path to the Diffusers layout | vllm-omni's HV1.5 SDPA path emits the Diffusers mask layout, or rollout and replay share one transformer implementation |
