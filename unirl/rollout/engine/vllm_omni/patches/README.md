...
| `patch_dit_lora_loader` / `patch_ar_lora_loader` | Stock `DiffusionLoRAManager._load_adapter` loads only from a file path; RL pushes freshly-trained adapter tensors without a disk round-trip (`OmniTensorLoRARequest`). Lifted verbatim from verl-omni, plus the single-thread pin around `lora.optimize()` — carry that pin into any replacement, it is not part of the tensor-bag reason | vllm-omni's LoRA managers accept tensor-bag requests natively |
...
| `patch_minimax_h3_packed_lora_mapping` (`compat_minimax_h3_lora`) | H3's DiT fuses both qkv and the SwiGLU gate/up (`mlp.fc1`), and `v0.27.0rc1` declares neither, so a trained adapter loads and activates against zero layers. Extends the model's `stacked_params_mapping` rather than replacing it, so the qkv half self-retires against the rows [vllm-omni #6476](https://github.com/vllm-project/vllm-omni/pull/6476) added | upstream declares the `mlp.fc1` → `gate_proj`/`up_proj` rows too (the qkv half is already redundant from 0.28.0rc1) |
| `patch_minimax_h3_canvas_short_edge` (`compat_minimax_h3_canvas`) | `_resolve_output_canvas` rejects any short edge but the released 768, so the engine cannot serve the 288x384 the DiffusionNFT recipe trains at — a canvas the trainside path runs happily. Replaces only that equality with a positive-multiple-of-32 check and keeps the ratio and max-pixel policy intact | vllm-omni takes a requested short edge, or H3 RL settles on 768-only geometry |
