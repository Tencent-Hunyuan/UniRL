...
| `patch_moe_workspace_pool` | vLLM's reusable MoE `WorkspaceManager` scratch buffer is allocated outside the `weights`/`kv_cache` CuMem pools, so `sleep()` leaves it resident and a colocated engine keeps that VRAM through the train phase. Routes workspace growth through a `moe_workspace`-tagged CuMem pool and registers the tag in `MultiprocExecutor.sleep`'s `sleeping_tags` so it offloads/wakes with the native pools. Self-skips when the allocator is absent (sleep mode off → `CuMemAllocator.instance is None`) | vLLM allocates the MoE workspace inside a sleep-managed pool itself |
