...
class QwenImageInputAdapter(DitInputAdapter):
    ...
    def build_sampling(self, sample: Sample) -> List[StageSampling]:
        ...
        # Qwen's CFG knob: set it ALWAYS so upstream's ``or 4.0`` default never fires (at <= 1.0 ``do_true_cfg`` stays False regardless of the prompt-side gate — belt and suspenders).
...
class QwenImageT2iAdapter(ModelAdapter):
    ...
    # The Qwen2.5-VL tokenizer lives in the tokenizer/ subfolder; the worker loads it and the single-stage path never calls build_prompt_tokens.
