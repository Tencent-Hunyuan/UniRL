...
class AgenticRolloutEngineConfig(BaseEngineConfig):
    ...
    # Inner single-turn rollout engine config (e.g. ``SGLangEngineConfig``); kept ``Any`` so it is built from its own ``_target_`` via ``inner.make_engine``.
    inner: Any
    # The environment (the world side of a turn) — an :class:`Environment`, kept ``Any`` so a ``_target_`` (e.g. ``ToolEnvironment`` + tools) is instantiated per worker by the walker. Must be re-entrant: one instance serves all of a worker's concurrent trajectories (LIN-522).
    env: Any

    max_turns: int = 8
    episode_sampling: Any = None
    # Max concurrent trajectories per worker — the drain thread-pool size (one trajectory per thread; a thread holds its trajectory across tool-wait). Set a small multiple of the inner backend ``concurrency`` so trajectories in tool-wait don't starve the GPU.
    per_worker_concurrency: int = 8
    # Partial rollout (LIN-531): expose the coordinator as a ``submit``/``poll``/ ``abort`` interface so the *trainer* can over-sample, commit the first ``batch_size`` complete GRPO groups, checkpoint the unfinished tail at a **turn boundary**, and carry it to the next round (resumed under new weights). ``False`` ⇒ ``abort`` never fires, so behaviour is byte-identical to the barrier ``generate``.
