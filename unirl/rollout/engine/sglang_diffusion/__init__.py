...
ServerArgs intent. Coexists with the legacy ``rollout/engine: sglang`` preset;
recipes opt in via ``cfg.rollout.engine=sglang_diffusion``.

Importing this package registers the engine config (``@register_config``) and
populates the adapter registry (the ``adapters`` import fires the
``@register_adapter`` side-effects).
"""
