...
def _apply_fused_param_mapping(module, named_tensors):
    ...
    * **plain renames** — ``{regex: replacement_str}`` — the model simply renamed
    ...
    # A leftover name that is still not a real model param slipped through every mapping branch (unmatched pattern, or a rename whose target does not exist). It will silently no-op in the exact-match loader — exactly the stale-weight failure this mapping is meant to prevent — so surface it loudly instead.
...
def patch_weights_updater() -> None:
    ...
    # Upstream module-globals the verbatim bodies depend on. Bound here so the nested fns resolve them through THIS scope identically to the fork.
    logger = wu.logger
    _load_weights_into_module = wu._load_weights_into_module

    # REPLACE module-level load_weights_into_model (+ remap helper) Upstream's load_weights_into_model has no LoRA name-remap. The fork added a bidirectional ``.base_layer.`` remap so weight-sync names match whether or not the model wrapped a layer with a ``.base_layer.`` indirection.
    if not getattr(wu.load_weights_into_model, _LWIM_SENTINEL, False):
        ...
        def _build_lora_name_remap(model_params: dict) -> dict:
            ...
            for param_name in model_params:
                if ".base_layer." in param_name:
                    ...
                    # Only add reverse remap for plausible base_layer patterns e.g. attn.to_q.weight → attn.to_q.base_layer.weight
        ...
        def load_weights_into_model(weights_iter, model_params: dict) -> None:
            ...
            # Silent weight-drop is dangerous: a name matching no model param is skipped above (``continue``), so a sender/receiver naming or fusion mismatch (diffusers separate ``to_q/to_k/to_v`` vs SGLang's fused ``to_qkv``) leaves those weights at their loaded base value with NO error — the rollout silently never sees the trained update. Fused projections are consumed upstream by ``_apply_fused_param_mapping``; anything still unmatched here is a real mismatch, surfaced loudly.
            if _skipped:
                logger.warning(
        ...
        # Note: upstream's ``_load_weights_into_module`` calls ``load_weights_into_model`` via its module global, so it picks up the replacement above; the ``_load_weights_into_module`` object itself is unchanged, so the local alias bound at the top stays valid.
    ...
    def _post_update_cleanup(
        ...
        # Handle LoRA state only after ALL buckets/dtypes are done (flush_cache=True). Calling per-bucket would corrupt state: partial updates + unmerge/merge cause base weights to be reverted or LoRA to be double-applied.
    ...
    def _apply_named_tensor_weights(
        ...
        for module_name, module in modules_to_update:
            ...
            if not module_tensors:
                continue
            try:
                # Fused-projection models (Z-Image: to_q/k/v -> to_qkv, feed_forward.w1/w3 -> w13) need the model's param_names_mapping applied so the trainer's separate-projection tensors land in the right fused shard. Consume those here; the rest fall through to the exact-match loader below.
