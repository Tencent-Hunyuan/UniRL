...
# Condition fields carried from the worker result. FLUX.2 replay needs both the condition tokens and their ids, and the grid factorization can't be recovered from N alone (see the Flux2KleinAdapter.build_condition note), so the ids are captured rather than recomputed.
_COND_FIELDS = (
    "prompt_embeds",
    "audio_prompt_embeds",
...
# result(Req) source attr -> OutputBatch dest attr (the fork's gpu_worker mapping). The response translator mounts it only when its fused length matches the fused embeds (``utils.tracks.fuse_text_conditions``): Qwen-Image's single-encoder mask matches and flows through to replay; SD3's per-encoder mask fuses to 410 vs the merged 333-token embeds, so it is dropped there (SD3's ``predict_noise`` ignores the mask anyway — see the historic ~68x LoRA-gradient dilution that motivated this guard).
_POS_MAP = {
...
# Dest fields whose per-encoder tensor may arrive un-batched ``[seq, hidden]``. Only these dests get a batch dim added at ingestion; pooled (``[B, hidden]``) and masks (``[B, seq]``) are already batched and must be sliced/merged as-is.
...
def patch_conditions() -> None:
    ...
    # Field injection on the two dataclasses that carry the conditions. Both are registered in ``__dataclass_fields__`` (so they round-trip through ``fields`` / ``replace`` / ``asdict`` / pickle) AND have their generated ``__init__`` wrapped to strip-then-reapply the injected keys -- because once a field lives in ``__dataclass_fields__``, ``dataclasses.replace`` passes EVERY field as a kwarg to ``__init__``, and the frozen generated ``__init__`` would reject the post-hoc ones.
    ...
    # Disagg/raw-Req path: copy embeds Req -> OutputBatch in the per-Req conversion (this seam never fires on the monolithic decoding path).
...
def _inject_dataclass_fields(cls, sentinel: str, *, type_str: str) -> None:
    ...
    for name in _COND_FIELDS:
        ...
        # Class-level default so getattr(obj, name) works even when our write sites did not set it (flags off).
...
def _copy_conditions(src, output_batch) -> None:
    ...
    # Edit-Plus image_latent: a single packed [B, S_img, C*4] tensor set by upstream's ImageVAEEncodingStage. Not gated on a SamplingParams flag — presence on the batch IS the gate (only Edit-Plus sets it; T2I leaves it None).
    ...
    # Edit-Plus vae_image_sizes: list[tuple[int, int]] of pixel (W, H) pairs from upstream's preprocess_vae_image. The adapter needs these to unpack image_latent to spatial form.
    ...
    # FLUX.2-Klein ti2i condition_image_latent_ids: a single [B, N, 4] RoPE-id tensor set by upstream's prepare_condition_image_latent_ids alongside image_latent. Same presence-is-the-gate + [tensor]-wrap contract as image_latent; None for every non-Klein-ti2i adapter.
...
def _coalesce_duplicate_single_sample_encodes(value):
    ...
    if any(tuple(t.shape) != first_shape for t in value[1:]):
        return value
    # Value check: only coalesce when the same-shape entries are actually byte-identical (the shared-list duplicate signature). Same-shape-but- differing tensors are a genuine multi-encoder whose outputs must NOT be collapsed — hardens the shape-only heuristic per the examples/ README note.
    ...
    for name in _COND_FIELDS:
        ...
        for enc_idx in range(num_encoders):
            ...
            if any(t is None for t in tensors):
                ...
                # Non-tensor (list[tuple[int,int]]); all outputs in a group share the same source image, so take the first.
    ...
    def _result_common(req, output_batch, generation_time, output_index=None):
        ...
        for name in _COND_FIELDS:
            ...
            if name == "image_latent_sizes":
                # image_latent_sizes is list[tuple[int,int]] per encoder (NOT per-output — all outputs in a group share one source image). _merge_conditions already took tensors[0] across batches, so the merged value has exactly one entry per encoder. Slicing by output_index would index past it (e.g. [(1024,1024)][3:4] = []) → "got 0 source images" in _collect_image_latents.
                common[name] = val
            else:
