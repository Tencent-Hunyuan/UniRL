...
def patch_platform_device() -> None:
    ...
    def get_available_gpu_memory(
        ...
        if torch.distributed.is_initialized():
            ...
            # Colocate: the FSDP rank may exceed this actor's visible device count (CUDA_VISIBLE_DEVICES restricts the actor to 1 GPU). Only adopt the rank as the device id when it is a valid local device; otherwise honor the caller-passed (local) device_id.
