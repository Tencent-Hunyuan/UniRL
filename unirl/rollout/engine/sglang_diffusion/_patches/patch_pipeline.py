...
def patch_pipeline() -> None:
    ...
    def forward_batch(self, batches, server_args):
        # Grouped path (len>1) skips the residency-manager setup that `forward` does; mirror it so executor.execute_group_with_profiling has a manager.
        if len(batches) > 1:
    ...
    # The grouped executor passes a LIST of batches to the residency hook, but ComponentResidencyManager.begin_request reads ``batch.is_warmup`` (a single Req) -> ``'list' object has no attribute 'is_warmup'``. All grouped batches share is_warmup (same request group), and before_stage/after_stage don't touch batch, so unwrap to a representative batch[0].
