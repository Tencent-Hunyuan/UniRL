...
class MochiAdapter(ImageAdapter):
    ...
    # Legacy image-path video family: drop 4-D decoded samples (incl. single-frame) rather than squeezing them into images.
...
class Ltx2T2VAdapter(VideoAdapter):
    ...
    def build_condition(self, results: List[RawResult]) -> Dict[str, Any]:
        ...
        # SGLang's LTX connector replaces padded positions with learned registers and gives the DiT an all-valid post-connector mask (the native LTX-2.3 path passes no mask at all). ``patch_conditions`` emits the pre-connector tokenizer mask for generic families, so carrying it into replay would incorrectly mask those registers.
        if text is not None:
    ...
    def build_segment(
        ...
        *,
        ...
        # LTX-2 co-denoises an AUDIO latent the video DiT cross-attends to; collect the parallel audio trajectory and stamp it as ``segment.aux_latents`` so the trainside ``LTX2DiffusionStage.replay`` replays the same per-step audio (else it raises "aux_latents (audio trajectory) missing").
