...
class CPPOConfig(BaseAlgorithmConfig):
    ...
    # "rollout" (default) keeps the rollout engine's emitted logprobs as mu for ALL num_updates_per_batch steps (verl bypass-mode parity, = the behavior policy CPPO anchors on); "replay" freezes a train-side pi_old at pre-update weights in prepare_segment instead.
...
def _cppo_mask(
    *,
    ...
    """
    # The mask is a no-grad gate, so compute the divergence/prefix arithmetic in fp32 regardless of the logprob dtype: torch.quantile rejects bf16/fp16, and a bf16 cumsum over a long response (up to max_new_tokens) loses the prefix sum to rounding. The recipe pins logprob_precision=fp32, but this keeps CPPO correct under a bf16-logprob config too (where DRPO has no cumsum/quantile).
    ...
    for D_t, toward_t in zip(
        ...
        # Decreasing position weight w_t in [w_min, 1] over this sequence's own length T (paper Eq. 9); t is 1-based, so w_1 = 1 and w_T = w_min.
        pos = torch.arange(1, T + 1, device=D_t.device, dtype=D_t.dtype)
        w_t = 1.0 - (1.0 - w_min) * (pos - 1) / max(T - 1, 1)
        Z_t = w_t * D_t

        # Prefix sums with a one-token right shift so the decision at t uses only the preceding prefix (S_{t-1}, W_{t-1}); S_0 = W_0 = 0 (Appendix D).
...
def _cppo_loss(
    *,
    ...
    metrics = {
        ...
        # Fraction of tokens rejected by the CPPO budget mask (paper Fig.
...
class CPPO(StageAlgorithm):
    ...
    # old_logp (the ratio denominator and the Binary-TV mu) is the rollout (SGLang) log-prob, frozen on the segment and unchanged across mini-batch updates — so reusing it across num_updates_per_batch>1 is the deliberate rollout-anchored trust region (verl bypass_mode=True parity), matching GRPO / DRPO.
    ...
    def __init__(
        ...
        *,
        ...
        # pi_old / mu source. "rollout" = the rollout engine's emitted segment.log_probs, kept as the anchor for ALL num_updates_per_batch steps (= the behavior policy CPPO's trust region anchors on; verl bypass-mode parity). "replay" = prepare_segment recomputes a frozen train-side pi_old at pre-update weights. Both anchors are frozen for the whole rollout batch, so multi-update is supported in either mode.
    ...
    def prepare_segment(
        ...
        *,
        ...
        # Keep the replay's native (fp32) precision so the anchor stays as close as possible to new_logp's fp32 replay (mirrors DRPO / FlowGRPO).
