...
class ZImageBundle(Bundle):
    ...
    def from_config(cls, config: ZImagePipelineConfig) -> "ZImageBundle":
        ...
        # AutoModel (not a hardcoded Qwen3Model) mirrors the official Z-Image loader: it instantiates the base encoder named in the checkpoint's text_encoder/config.json, so a future text-encoder swap just works.
        text_encoder = (
