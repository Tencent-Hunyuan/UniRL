...
# LTX-2 stretches the σ grid so its terminal value is 0.1 (scheduler_config ``shift_terminal``); the diffusers scheduler applies this in set_timesteps.
...
class LTX2SchedulePolicy(FlowMatchSchedulePolicy):
    ...
    def compute_mu(self, image_seq_len: int, num_inference_steps: int) -> float:
        # image_seq_len is intentionally ignored: diffusers pins it to max_image_seq_len, so the linear interpolation degenerates to max_shift.
