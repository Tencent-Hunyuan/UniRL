...
class Qwen3ChatTemplateStage(EmbedStage[Qwen3ChatInput, Qwen3ARConditions]):
    ...
    def __init__(
        ...
        *,
        ...
        # MUST agree with the rollout engine's chat template (rollout.config.chat_template_kwargs.enable_thinking) or train/rollout prompts diverge and the importance ratio breaks.
    ...
    def embed_messages(
        ...
        *,
        ...
        for messages, sample_tools in zip(conversations, tools):
            ...
            # transformers v5 flipped apply_chat_template's `return_dict` default from False to True: it now returns a BatchEncoding, and integer- indexing a fast-tokenizer BatchEncoding yields a tokenizers.Encoding (no `.to`), not a tensor. Pin return_dict=False so we keep the bare input_ids [1, L] tensor identically across v4/v5; squeeze leading dim.
            ids = ids[0]
