...
def register_exact_actor_provider(*, context_factory, log_softmax) -> None:
    ...
    if _EXACT_CONTEXT_FACTORY is not None and (
        _EXACT_CONTEXT_FACTORY is not context_factory or _EXACT_LOG_SOFTMAX is not log_softmax
    ):
        raise RuntimeError("a different exact actor provider is already registered in this process")
