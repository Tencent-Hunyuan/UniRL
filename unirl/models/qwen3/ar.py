...
def _packed_replay_supported() -> bool:
    """Feature-detect the packed varlen replay prerequisites (review #43 B1/B2).
    ...
    dense path instead. ``UNIRL_DISABLE_PACKED_REPLAY=1`` is the explicit kill
    switch for A/B and rollback.
    """
    import os as _os
        ...
        *,
        ...
        if batch_size > 1 and segment.lengths is not None and _packed_replay_supported():
            ...
            if attn_impl != "flex_attention":
                target = L  # bucketing only pays for flex's per-shape compile; sdpa would just pay filler FLOPs
            if target > L:
                n_fill = target - L
