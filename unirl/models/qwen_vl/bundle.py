...
class QwenVLBundle(Bundle):
    ...
    def from_config(cls, config: QwenVLPipelineConfig) -> "QwenVLBundle":
        ...
        if config.meta_init_transformer:
            # Meta-init (FSDP / VeOmni load_sharded path): parameters on the meta device, materialized + loaded by the backend from the checkpoint root after sharding (the embedded ViT is part of the trainable tree, loaded with it — not a separate aux). build_meta_init_transformer keeps buffers/attrs real on CPU: HF rotary inv_freq is a non- persistent buffer computed in __init__ and absent from the checkpoint, so to_empty later clobbers it -> garbage RoPE.
        ...
        # Structural (sets requires_grad / checkpointing flags, no weight access); runs on both builds and persists through to_empty + load.
