...
class QwenVLChatTemplateStage:
    def __init__(
        ...
        *,
        ...
        # When True, pad every prompt to a fixed `max_prompt_length` instead of the per-batch dynamic max. Required by the v2 DP trainer: shards from different rollout workers are concatenated (dim 0) at merge time, so input_ids/attention_mask must share one sequence length across shards.
    ...
    def embed(
        ...
        # Per-sample lists for pixel_values and image_grid_thw. Using per-sample lists with FieldKind.CONCAT ensures correct concatenation when multiple rollout workers' conditions are merged.
