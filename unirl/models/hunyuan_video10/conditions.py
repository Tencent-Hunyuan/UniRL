"""HunyuanVideo10Conditions — ``text_llama [B, seq, 4096]`` plus ``pooled_clip`` embeds ``[B, 768]``."""
...
@dataclass
class HunyuanVideo10Conditions(Batch):
    ...
    @classmethod
    def from_dict(cls, d: Dict[str, Condition]) -> "HunyuanVideo10Conditions":
        ...
        if not isinstance(text_llama, TextEmbedCondition):
            raise TypeError(
                f"HunyuanVideo10Conditions.from_dict: expected d['text_llama'] "
        ...
        if not isinstance(pooled_clip, TextEmbedCondition):
            raise TypeError(
                f"HunyuanVideo10Conditions.from_dict: expected d['pooled_clip'] "
                f"to be a TextEmbedCondition, got "
    ...
    def to_dict(self) -> Dict[str, Condition]:
        ...
        if self.text_llama is None or self.pooled_clip is None:
            raise ValueError(
                "HunyuanVideo10Conditions.to_dict: both text_llama and pooled_clip "
...
__all__ = ["HunyuanVideo10Conditions"]
