...
class QwenImageBundle(Bundle):
    ...
    def from_config(cls, config: QwenImagePipelineConfig) -> "QwenImageBundle":
        ...
        # Node-local load serialization: 8 colocated ranks each hold ~20 GiB anon RSS while materializing the 20B transformer (safetensors -> bf16 staging). The simultaneous burst blows the pod's k8s memcg limit (~439 GiB incl. page cache) and the kernel OOM-kills raylet/python (LIN-382 qwen probes b/d: "Memory cgroup out of memory", anon-rss ~20-23 GiB per kill).
        ...
        try:
            ...
            if lock_file is not None:
                # Return this rank's staging anon to the kernel before the next rank starts its load, so the serialized peak holds.
                import gc
    ...
    def _from_config_locked(cls, config: QwenImagePipelineConfig) -> "QwenImageBundle":
        ...
        if config.meta_init_transformer:
            # Meta-init (VeOmni load_sharded path): params on meta, weights load post-parallelize. Qwen-Image's QwenEmbedRope holds its complex rope tables (pos_freqs / neg_freqs) as plain __dict__ tensors, so to_empty never materializes them; build_meta_init_transformer keeps them real on CPU and captures them into meta_init_state for load_trainable_weights to restore after the sharded load.
        ...
        if config.meta_init_transformer:
            # Consumed by VeOmniBackend's post-parallelize weight load. Kept as the raw join — the backend validates local-dir-ness at load time (HF repo IDs need a local download first).
