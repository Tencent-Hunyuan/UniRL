...
# Pack / unpack helpers — module-level so unit tests can import them without constructing the stage.
...
class QwenImageDiffusionStep(DiffusionStep[QwenImageBundle, QwenImageConditions]):
    ...
    def predict_noise(
        ...
        *,
        ...
        # Qwen-Image's transformer takes raw sigma as the timestep input (not sigma * 1000 like SD3).
        if sigma.dim() == 0:
        ...
        # The transformer needs the per-sample latent grid shape so it can rebuild positional embeddings; format is ``[[(frames, H/(vae_scale_factor*2), W/(vae_scale_factor*2))]] * B``. Here ``latent_h`` / ``latent_w`` ARE already in the post-VAE spatial grid, so the patchify divisor is just 2.
        img_shapes = [[(1, latent_h // 2, latent_w // 2)]] * batch_size

        # Distilled-guidance scalar — embedded by the transformer when ``guidance_embeds=True`` is set on its config (set by some Qwen-Image variants only).
        guidance = None
        ...
        # Per-sample true text lengths — the RoPE builder slices its text frequency table by ``max(txt_seq_lens)`` (required positionally by the installed diffusers; passing only the attention mask raises ``max(None)`` TypeError — LIN-382 qwen probe-e). The embeds must be trimmed to this slice's true max first: replay microbatches carry the BATCH-wide pad width (e.g. 18) while their own max true length may be shorter (12) — diffusers applies RoPE over the full tensor width and the freq slice over max(txt_seq_lens), so a width mismatch hard-crashes in apply_rotary_emb_qwen (probe-f).
        ...
        if guidance_scale > 1.0:
            ...
            if neg is not None and neg.embeds is not None:
                ...
                # Combined-CFG with norm correction. Spec: keep the per-token L2 norm of the conditional prediction after CFG blending.
    ...
    def step(
        ...
        *,
        ...
        if latent_h <= 0 or latent_w <= 0:
            # Recover from sample shape — diffuse/replay always pass both explicitly, but defaulting here keeps unit tests that hand- roll ``[B, C, H, W]`` simple.
...
class QwenImageDiffusionStage(DiffusionStage[QwenImageConditions]):
    ...
    # Qwen-Image t2i uses an 8× VAE downsample and 16 latent channels in the post-VAE grid. The model bundle's ``transformer.config`` carries the authoritative count via ``in_channels // 4`` (the packed-latent format multiplies by 4); we default to that and let callers override via the stage constructor.
    ...
    def __init__(
        ...
        *,
        ...
        if latent_channels is None:
            # Read from the transformer config: in_channels is the packed-input dim (C * 4), so the post-VAE channel count is in_channels // 4. Falls back to 16 if the attr is missing.
    ...
    def diffuse(
        ...
        *,
        ...
        # Latent grid follows the diffusers QwenImagePipeline convention: latent_h = 2 * (H // (vae_scale_factor * 2)). The doubled-mod rounding makes sure (latent_h % 2 == 0), which the 2×2 patch pack requires.
