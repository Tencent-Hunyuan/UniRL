...
class BooguImageBundle(Bundle):
    ...
    def _from_config_locked(cls, config: BooguImagePipelineConfig) -> "BooguImageBundle":
        ...
        if config.meta_init_transformer:
            # Architecture only, on the meta device; the backend materializes per-rank shards after wrapping and loads weights from the stashed path. Boogu's vendored tree has zero registered buffers and zero init-computed plain-tensor attrs (rope freqs_cis is a per-call input), so the captured init state must come out EMPTY; if it ever grows after a re-vendor, that is the signal a quirk restore became load-bearing.
        ...
        # Separate-engine recipes can skip the frozen encoder copy on actors that never embed text (qwen_image precedent).
        text_encoder = None
        processor = None
        if config.load_text_encoder:
            # ``mllm/`` may ship the full CausalLM wrapper (reused upstream as a prompt rewriter). Conditioning uses hidden states only, so keep the inner ``Qwen3VLModel`` and free the wrapper/lm_head (mirrors pipeline_boogu.py:194-198).
