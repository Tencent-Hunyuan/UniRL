...
class Qwen3_5ChatTemplateStage:
    ...
    def __init__(
        ...
        *,
        ...
        # MUST agree with the rollout engine's chat_template_kwargs.enable_thinking or train/rollout prompts diverge and the importance ratio breaks.
        self.enable_thinking = bool(enable_thinking)
        # When True, pad every prompt to max_prompt_length (v2 DP trainer shard concatenation requires a single shared seq len across shards).
    ...
    def embed(
        ...
        def apply_template(
            ...
            *,
            ...
            if has_media:
                # Qwen3.5's AutoProcessor scans every message as multimodal content. Normalize system/text strings to text blocks so the scan does not iterate over characters.
                processor_messages = [
                    {
                        **message,
                        "content": (
            ...
            # The processor's multimodal pre-scan does not accept the standard string content emitted by build_text_messages. The tokenizer does, and is also the exact path used by the SGLang text adapter.
        ...
        for messages in conversations:
            ...
            if not has_media and prompt_len > self.max_prompt_length:
                # Plain right-truncation would drop the trailing generation-prompt suffix ("<|im_start|>assistant\n", plus thinking tokens when enabled), leaving the model unprimed to open an assistant turn → degenerate/empty generations. Truncate the body but keep the suffix: re-tokenize without the generation prompt to measure the suffix, then splice head + suffix at max_prompt_length.
