...
class JanusProImagePromptStage:
    ...
    def embed(self, texts: Texts, *, cfg_weight: float) -> JanusProImageARConditions:
        ...
        for text in texts.texts:
            ...
            if len(cfg_ids) > 2:
                cfg_ids[1:-1] = [pad_id] * (len(cfg_ids) - 2)
            prompt_rows.append(token_ids)
            cfg_rows.append(cfg_ids)

        return JanusProImageARConditions(
            prompt=_right_pad_rows(prompt_rows, pad_id=pad_id, device=self.bundle.device),
            cfg_prompt=_right_pad_rows(cfg_rows, pad_id=pad_id, device=self.bundle.device),
