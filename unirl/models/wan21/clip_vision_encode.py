...
class WAN21CLIPVisionEncodeStage(EncodeStage[Images, ImageEmbedCondition]):
    ...
    def encode(self, p: Images) -> ImageEmbedCondition:
        ...
        # Penultimate hidden state — matches diffusers WAN I2V upstream (``pipeline_wan_i2v.py::encode_image``); using ``last_hidden_state`` would shift the embedding distribution the transformer was trained against.
