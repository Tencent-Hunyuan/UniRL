...
class WAN21ImageLatentEncodeStage(EncodeStage[Images, ImageLatentCondition]):
    ...
    def encode(self, p: Images) -> ImageLatentCondition:
        ...
        # Latent temporal axis: pixel-space frames collapse to ``(num_frames - 1) // 4 + 1``. Caller has already gated on the ``(num_frames - 1) % 4 == 0`` invariant.
        ...
        with torch.no_grad():
            # ``.mode()`` (deterministic) vs ``.sample()`` — diffusers I2V uses ``retrieve_latents(..., sample_mode="argmax")`` which is ``.mode()``. ``.sample()`` would drift rollout/replay.
        ...
        # 4-channel first-frame mask (mirrors diffusers ``pipeline_wan_i2v.py:468-479``): 1.0 at first pixel-time slot, 0.0 elsewhere; then view-reshape + transpose so the temporal downsampling factor ends up on the channel axis.
        ...
        # Defensive: when ``num_frames=1`` the reshape above would produce T_lat=1; for the standard 5,9,13,... family it lines up.
