...
class WAN21Pipeline(Pipeline):
    ...
    def __init__(
        ...
        *,
        ...
        # Stages default to None and are built from the (trainer-injected) bundle — mirrors SD3Pipeline so the v2 trainer can construct the pipeline via ``remote_hydra(pipeline_cfg, bundle=self.bundle)`` without reloading weights.
        super().__init__()
        self.bundle = bundle
        self.text_embed = (
            text_embed
    ...
    def generate(self, sample: Sample) -> Sample:
        ...
        # Image conditioning needs diffusion geometry and is intentionally attached outside the public text-only builder.
