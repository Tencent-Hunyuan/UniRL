...
def restore_init_state(model: nn.Module, captured: Optional[dict]) -> int:
    ...
    for fqn, value in buffers.items():
        ...
        # After FSDP2 (fully_shard), non-persistent buffers can be DTensors; a plain ``live.copy_(cpu_tensor)`` onto a DTensor silently fails to write, leaving the ``to_empty`` garbage (e.g. RoPE ``inv_freq==0`` -> position-blind model -> rollout/replay logprob mismatch).
...
def recover_rope_inv_freq(model: nn.Module) -> int:
    ...
    for module_name, m in model.named_modules():
        ...
        if rope_type == "default":
            # Preserve the empirically verified Qwen3 default path used by the rollout engine; do not reinterpret a default config as scaled RoPE.
