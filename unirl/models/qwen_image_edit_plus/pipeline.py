...
class QwenImageEditPlusPipeline(Pipeline):
        ...
        *,
        ...
        if bundle.text_encoder is not None:
            text_embed = QwenImageEditPlusTextEmbedStage(
                ...
                # Honor a text-encoder override so the processor tracks the tokenizer/text encoder, not just the main checkpoint.
    ...
    def build_conditions(
        ...
        *,
        ...
        # CFG empty negative: single-space " " (mirrors base Qwen-Image — the chat-template prefix strip makes "" unsafe).
        if negatives is None and float(guidance_scale) > 1.0:
            negatives = Texts(texts=[" "] * len(texts.texts))
        # Both CFG branches share the source image when multimodal prompt conditioning is enabled. Disabling it preserves the Edit-Plus template/drop-64 path while passing no image to the text encoder.
