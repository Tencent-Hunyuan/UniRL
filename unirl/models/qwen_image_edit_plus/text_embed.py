...
# Edit chat template + drop index (upstream diffusers ``QwenImageEditPlusPipeline``: ``prompt_template_encode`` / ``prompt_template_encode_start_idx``). The system prompt differs from base Qwen-Image, so the fixed prefix is 64 tokens (not 34).
PROMPT_TEMPLATE = (
...
# Condition-image grid fed to the text encoder (upstream ``CONDITION_IMAGE_SIZE``). NOTE: this is the low-res condition size for the *text encoder*; the VAE latent-concat path resizes the same source image to a separate ≈1024² grid.
...
class QwenImageEditPlusTextEmbedStage(ImageConditionedEmbedStage[Texts, Images, TextEmbedCondition]):
    ...
    def __init__(
        ...
        *,
        ...
        # Same root as text encoder / tokenizer: honor ``processor_path`` (from ``text_encoder_ckpt_path``) else the main checkpoint.
    ...
    def _encode(self, prompts: List[str], images: Optional[Images]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Upstream always goes through the processor; when image is None it still tokenizes text and omits pixel_values / image_grid_thw.
        ...
        # Strip the 64-token edit-template system prefix (with or without image-placeholder tokens after it).
        ...
        # Final slice to the configured budget (image-placeholder tokens are at the front when present, so a short prompt keeps its visual context).
