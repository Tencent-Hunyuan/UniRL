...
class Flux2KleinConditions(Batch):
    ...
    # Image-edit / reference conditioning: the source image, VAE-encoded and packed into transformer tokens ``[B, N, 128]``, with its 4-axis RoPE position ids ``[B, N, 4]`` (time-offset from the noise latents so the transformer can tell condition tokens apart).
