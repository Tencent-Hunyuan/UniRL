...
class HunyuanImage3Bundle(Bundle):
    ...
    def __init__(
        ...
        *,
        ...
        # ``vae`` / ``vit`` are exposed as ``@property`` over ``self._vae`` / ``self._vit`` so :meth:`materialize` can flip them after meta-init construction (where they're initially ``None``). The backing transformer always has ``transformer.vae`` / ``transformer.vision_model`` children — they may be on meta until materialized.
        self._vae = vae
        self._vit = vit
        self.tokenizer = tokenizer
        self.scheduler = scheduler
        self.dtype = dtype
        # Used by :meth:`materialize` so the loaded VAE lands at the same dtype as ``from_config``'s eager path.
    ...
    def from_config(cls, config: HunyuanImage3PipelineConfig) -> "HunyuanImage3Bundle":
        ...
        # Shared MoE backbone — operates in both gen_text and gen_image modes. Must be ``AutoModelForCausalLM`` so we get ``HunyuanImage3ForCausalMM`` (with .vae / .vision_model / ._tkwrapper); ``AutoModel`` returns the inner ``HunyuanImage3Model`` (backbone only).
        ...
        # The 3D-VAE and SigLIP2 ViT are bundled inside the same checkpoint repo as auxiliary modules — the upstream code attaches them on the main model, so we expose direct references rather than reloading.
        ...
        # Real upstream attribute is ``vision_model`` (hunyuan.py:1729). The other names are kept for forward-compat with older / forked weights.
        vit = (
    ...
    def from_meta_config(
        ...
        # Build full model on meta. ``init_empty_weights`` puts every ``nn.Parameter`` allocation on the meta device, so no weight memory is allocated.
        with init_empty_weights():
            transformer = AutoModelForCausalLM.from_config(hf_config, trust_remote_code=True)

        # ``from_config`` does NOT load ``generation_config.json`` (only ``from_pretrained`` does). Falls back silently if the file is missing (the HF default GenerationConfig is fine for non-HI3 paths).
        try:
        ...
        # VeOmni's ``parallelize`` calls ``init_weights()`` on the module it wraps (the decoder, via :meth:`trainable_module`) right after ``to_empty``; stamp it to a no-op so it cannot clobber the real weights that :meth:`materialize` installs post-shard. Harmless on the FSDP path (never called there).
    ...
    # Wrapper-level diffusion-head modules: siblings of ``transformer.model`` (the FSDP-wrapped decoder) that are part of the diffusion forward path. Always materialized — they're tiny vs. the decoder and required for replay.
    _DECODER_HEAD_ATTRS = (
        "lm_head",
        "final_layer",
        "patch_embed",
    ...
    def materialize(
        ...
        *,
        ...
        # Plan: (top-level attr name on transformer, module). Decoder is always first.
        ...
        # Build filtered state_dict on rank 0 only.
        if _current_rank() == 0:
            ...
            # [Bug B fix] LoRA key rename: peft.inject_adapter_in_model wraps q/k/v/o_proj.weight as q/k/v/o_proj.base_layer.weight. With strict=False, set_model_state_dict silently skips the mismatch → meta-allocated NaN params stay.
        ...
        # Expert parallelism: pull the fused expert tensors OUT of the DCP load. They are EP-sharded DTensors (Shard(0) on the EP mesh); DCP's broadcast path copies the full [E,...] onto the local [E/ep,...] shard and fails. load_ep_experts re-shards them with the param's own mesh instead.
        expert_sd = {}
        if getattr(self, "_ep_enabled", False):
        ...
        if getattr(self, "_ep_enabled", False):
            ...
            # VeOmni root-shards the non-layer params (wte, ln_f, lm_head) into DTensors; HI3's ForCausalMM wrapper calls them OUTSIDE the decoder forward (wte -> inputs_embeds for ViT scatter; ln_f/lm_head -> logits), hitting `aten.* got mixed Tensor and DTensor`. Pass the whole transformer so lm_head (on the outer ForCausalMM) is reachable too.
        ...
        # [Bug B fix] Post-load validation: verify all LoRA base_layer params are finite and not on meta.
        if _current_rank() == 0:
        ...
        # Post-load casts + freeze + eval for aux modules. Aux needs its target_dtype restored — ``to_empty`` preserves the meta tensor's dtype (which after the FSDP-policy pre-wrap homogenization is bundle.dtype = bf16 across the board, but vae must run in vae_dtype for numerical reasons).
        if "vae" in aux_set:
