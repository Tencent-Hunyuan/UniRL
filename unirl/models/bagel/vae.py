...
class BagelVAEDecodeStage(DecodeStage[LatentSegment, Images]):
    ...
    def __init__(self, bundle: "BagelBundle", *, decode_batch_size: int = 4) -> None:
        self.bundle = bundle
        # Chunk the VAE decode along the batch axis. The fp32 decoder's upsample conv2d peaks at ~1GB/image, so decoding 24 at once OOMs a 7B-resident card; 4/chunk bounds the peak.
    ...
    def decode(
        ...
        *,
        ...
        def _decode(lat: torch.Tensor) -> torch.Tensor:
            ...
            # Decode in batch-axis chunks to bound the fp32 upsample-conv peak (per-image independent; cat keeps the [N, 3, H, W] order).
        ...
        # Framework convention (qwen_image / sd3 / flux2_klein): the VAE stays fp32 after the first decode — the .to(float32) above is a one-time lazy upcast, a no-op on later calls. The shared encode path is dtype-safe regardless: pipeline.py casts encode inputs/outputs at the vendor boundary, so the downstream bf16 vae2llm is unaffected.
        pixels = (decoded * 0.5 + 0.5).clamp(0.0, 1.0)
        # Move to CPU before returning: decoded pixels are only ever consumed as CPU PIL (reward scoring via tensor_frame_to_pil, rollout dump) and the flow algorithm uses latents, not decoded images. Keeping them on GPU makes the reward-step ray.get() gather deserialize onto the driver's cuda:0 (stacked on the rank-0 worker) → OOM at 32-GPU scale where the gathered batch is 4× the 8-GPU smoke.
