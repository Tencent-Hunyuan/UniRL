...
# LoRA targets = the BAGEL MoT generation-expert projections (flow_grpo train_bagel.py:425-433). These are the only modules trained in the gen-only FlowGRPO setup; the und (understanding) path stays frozen.
...
class BagelPipelineConfig:
    ...
    # Stage-level precision / numerical policy (operator/runtime knobs, not per-request shape). bf16 trajectory matches flow_grpo's BAGEL setup; the SDE log-prob step itself runs in fp32 inside the vendored kernel.
    ...
    # Latent geometry (FLUX-style VAE + patchify). Needed by latent_shape and the VAE decode/unpatchify.
    latent_patch_size: int = 2
    max_latent_size: int = 64
    vae_downsample: int = 8
    latent_channels: int = 16

    # Load the und ViT tower (``visual_und=True``): SiglipVisionConfig from ``<ckpt>/vit_config.json`` + SiglipVisionModel/connector/vit_pos_embed weights from the same ``ema.safetensors``. Required for image-INPUT tasks (it2i editing / i2t / it2t); the default False keeps the T2I-only construction byte-identical to before.
    enable_vit: bool = False

    # Trainable module is ``model.language_model`` (the MoT). Used only by dedicated-sync modes; trainside performs no weight sync.
    weight_sync_param_name_prefix: str = "language_model."

    use_lora: bool = False
    lora_target_modules: Tuple[str, ...] = BAGEL_MOE_GEN_LORA_TARGETS

    # T2I prompt-context cache: dedup the prompt prefill across the N GRPO siblings (BAGEL navit bs=1 sends them one per generate call). Safe ONLY when the und/prefill path is frozen (gen-only LoRA); BagelPipeline additionally auto-disables it at runtime if any non-gen param is trainable, so this flag is just the opt-out / cache-size knob.
