...
- Fractional no-MPS (one GPU): 1,620/1,620 valid and 27/27 parity checks
  passed; mean rewards/GPU-hour improved 20.0% over dedicated.
- MPS at 100% (one GPU): 1,620/1,620 valid and 27/27 parity checks passed;
  mean raw throughput improved 16.3% over fractional no-MPS and mean
  rewards/GPU-hour improved 39.6% over dedicated.
- Consolidated (one GPU): 1,620/1,620 valid and 27/27 parity checks passed;
  mean rewards/GPU-hour improved 102.0% over dedicated.
- All 81 candidate run-level parity checks passed at `atol=1e-3`; the maximum
  absolute score delta was `4.56e-4`. MPS service and benchmark logs contained
  no explicit FAULT, OOM, or restart event.
