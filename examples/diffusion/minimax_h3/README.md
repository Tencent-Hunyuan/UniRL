...
- **Scoring reads one frame.** `quality100` was qualified against uniform
  sampling over 9 frames, but `T2AVCompositeSpec` carries no multi-frame
  aggregation, so these recipes score the middle frame alone.
