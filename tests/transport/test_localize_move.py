...
def test_move_full_block(multigpu_probe):
    ...
    assert torch.equal(moved.local(), p.h.expected(6, 4, 100))
...
def test_move_partial_spans(multigpu_probe):
    ...
    assert torch.equal(moved.local(), exp)
...
def test_dedup_shared_received_handle(multigpu_probe):
    ...
    assert torch.equal(out[0][0][0].local(), p.h.expected(4, 4, 500))
...
def test_grad_preserved_when_sibling_moves(multigpu_probe):
    ...
    assert torch.equal(g_for.local(), p.h.expected(3, 4, 700))
