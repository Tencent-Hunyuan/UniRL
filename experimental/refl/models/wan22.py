...
# Matches the mainline module-level constant in unirl/models/wan22/diffusion.py. We do not import the private symbol so the recipe stays decoupled from the mainline's private surface.
    ...
    def diffuse_with_grad(
        ...
        *,
        ...
        for i in range(T):
            ...
            # Per-step KL against the LoRA-disabled reference, on the conditional prediction of the currently active branch. When CFG is enabled, this intentionally excludes the stop-grad uncond mix.
            ...
            # Keep the transition outside autocast and under the same BPTT window used by WAN21: grad-enabled for train steps, no-grad before ``mid_timestep``.
