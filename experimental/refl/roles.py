...
class ReflActorRole(Remote):
    ...
    def initialize(self) -> None:
        ...
        # Default PG over the actor role's workers (env:// from Remote.setup's dist_env); FSDP2 fully_shard wraps over it. Phase-0-validated order.
        ...
        # FSDP-wrap pipeline.bundle.transformer in place + LoRA + optimizer. The pipeline's stages reference the same bundle, so sampling runs through the wrapped trainable transformer.
    ...
    def generate_samples(
        ...
        *,
        ...
        # Single KL knob: the actor owns kl_weight (loss-side), the sampling config owns only sampling-shape knobs (mid/final window). A stale sampler_kwargs.kl_weight is an error, not a silent override.
        ...
        # Fixed init noise across rollouts and ranks — the contributor's verified DRaFT regime (params.seed used verbatim by the stage). Varying noise per rollout is a training-semantics change; do not introduce it here without its own evidence.
