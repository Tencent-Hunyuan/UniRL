...
def _cfg_get(config: Any, name: str) -> Any:
    ...
    # Fall back to whatever the top-level had (possibly None) so callers that tolerate ``None`` (e.g. ``pad_token_id``) keep working.
    if hasattr(config, name):
...
class Qwen2VLRewardModelBT(Qwen2VLForConditionalGeneration):
    ...
    def __init__(
        ...
        # When special tokens are configured, the pooling mode is forced — otherwise the trainer-set ``reward_token`` setting on disk wins.
        if self.special_token_ids is not None:
            self.reward_token = "special"

    # The forward code below addresses ``self.visual``; transformers 5.6 owns the vision tower at ``self.model.visual``. Read via ``_modules`` to avoid recursing into ``nn.Module.__getattr__``, which would re-trigger this property with a misleading "no attribute" error.
    @property
    ...
    def forward(
        ...
        # Visual + text token fusion. Identical to the upstream Qwen2VLForConditionalGeneration.forward except we don't compute an LM head — only the regression head over the final hidden states. transformers 5.6: the vision tower returns BaseModelOutputWithPooling whose pooler_output holds the merged features that fill the media placeholders (what get_video_features reads). last_hidden_state is the PRE-merger states (4x tokens, vision dim) — never usable here.
        ...
        if self.reward_token == "last":
            ...
            # Each sample has exactly 3 special tokens (VQ/MQ/TA), each producing an ``output_dim``-row of logits. Reshape → (B, 3, output_dim); when output_dim=3 we keep the diagonal (matching the special-token training contract).
