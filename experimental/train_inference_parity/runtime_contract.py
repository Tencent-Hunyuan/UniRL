...
EXPECTED_SYNC_TARGET = "experimental.train_inference_parity.sync.ParityIPCWeightSync"
...
class ParityRuntimeContract:
    ...
    def flags(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "logprobs_mode": _select(self.cfg, "rollout.config.logprobs_mode"),
    ...
    def validate(self) -> None:
        ...
        expect("parity.max_prompt_length", 256)
        expect("parity.max_new_tokens", 1024)
        ...
        for path in ("rollout.config.top_k", "sampling.top_k"):
            expect(path, 0)
        expect(
            "rollout.config.logprobs_mode",
            "processed_logprobs",
        )
        expect("rollout.config.dtype", "bfloat16")
        expect("rollout.config.enforce_eager", True)
        ...
        if batch_size <= 0 or samples_per_prompt <= 0:
            ...
            errors.append("num_devices must be positive")
