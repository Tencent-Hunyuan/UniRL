...
def _use_direct_layerwise_reload(layer) -> None:
    """Load the column-sharded o-proj without meta staging; see the plugin README.md."""
...
def _column_weight_loader(self, parameter, loaded_weight):
    ...
    if rows % world or tuple(loaded_weight.shape) != (self.output_size, self.input_size):
        raise ValueError(f"invalid full o-proj weight shape: {tuple(loaded_weight.shape)} for TP{world}")
    shard = rows // world
