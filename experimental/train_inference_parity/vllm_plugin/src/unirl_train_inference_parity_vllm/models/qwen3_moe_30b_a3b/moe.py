...
def _routed_experts(experts):
    return getattr(experts, "routed_experts", experts)
...
def _get_w2_column(experts) -> torch.Tensor:
    ...
    stock = _routed_experts(experts).w2_weight
...
def _expert_forward(block, hidden_states: torch.Tensor) -> torch.Tensor:
    ...
    routed_experts = _routed_experts(block.experts)
    slots = int(topk_ids.numel())
    if slots == 0:
        return torch.zeros_like(hidden)
    max_count = max(int(value) for value in counts)
    local_gate_width = int(routed_experts.w13_weight.shape[1])
    ...
    for expert, count_value in enumerate(counts):
        ...
        if count == 0:
            ...
            active = linear(
                rows,
                routed_experts.w13_weight[expert],
                None,
            )
