...
def _aggregate_parity_metrics(result: Any) -> tuple[Dict[str, Any], list[int]]:
    micros = [
        dict(getattr(micro, "metrics", {}) or {})
        for micro in getattr(result, "micros", ()) or ()
        if all(name in (getattr(micro, "metrics", {}) or {}) for name in CANONICAL_PARITY_METRICS)
    ]
    if not micros:
        ...
        return {name: _jsonable(metrics[name]) for name in CANONICAL_PARITY_METRICS}, []
