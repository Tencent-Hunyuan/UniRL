...
def _run_t2i_benchmark(spec: BenchmarkSpec, tag: str, args, resolved: Optional[checkpoints.ResolvedCkpt]) -> None:
    ...
    if args.stage in ("all", "generate"):
        ...
        # Spec generation defaults (diffusers kwarg names, e.g. geneval2 -> 512px/40-step/cfg1.0) under CLI overrides. Without this, t2i benchmarks silently used the diffusers pipeline defaults (~28 steps / cfg 7.0 / 1024px), a top cause of the issue #221 mismatch.
    ...
    if args.stage in ("all", "score"):
        ...
        # Config 2 (optional): simulate a distributed eval of WxB that repeats the last partial wave of batches to fill the world size, so a fixed prefix of prompts is double-counted.
...
def _run_text_benchmark(spec: BenchmarkSpec, tag: str, args) -> None:
    ...
    if args.stage in ("all", "score"):
        ...
        # Missing completions are a hard error (mirrors the t2i missing-images check): folding them in as acc=0 would silently deflate the score after a partial generate (a --num-prompts smoke run, or FAILED requests).
