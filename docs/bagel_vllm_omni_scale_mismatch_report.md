...
| Post-r3 memory/lifecycle controls | eight-round functional/lifecycle gate passed; physical gate failed | r5 completed eight rounds, repeatedly cleared 48.620 GiB of gradients, parked/restored 97.239 GiB of optimizer state with no pending slot, and entered ninth generation, but its external physical-memory floor was only 622 MiB |
...
| 32-device production | not run | encoded scale remains `P=32, N=24, M=1, U=2`; an earlier one-node run completed eight functional rounds but failed the physical-memory gate, while capped R12 passed the two-round capacity gate and third wake; 32-device behavior is unmeasured |
...
622 MiB physical headroom. Rejected candidate `094cfec0` used
