...
#
# The driver is one of the recipe packages, selected with ENTRY:
#   diffusion (default)  recipes/diffusion/configs/ (sd3_*, wan2*, qwen_image_*)
#   ar                   recipes/ar/configs/ (qwen_vl_grpo_*, qwen3_drpo_*)
#   pe                   recipes/pe/configs/ (prompt-enhancement joint diffusion+AR)
#   unified_model                  recipes/unified_model/configs/ (HunyuanImage3, unified AR+diffusion)
#
# The first positional arg is the recipe config name, relative to the recipe configs/ (e.g. sd3/sd3_trainside)
...
#   LAUNCH=ssh bash scripts/run_experiment_multinode_taiji.sh sd3/sd3_sglang_rollout_colocate
#   # VLM/AR recipe (4x8):
#   ENTRY=ar bash scripts/run_experiment_multinode_taiji.sh qwen_vl_grpo_geo3k_mc_4x8
#
set -euo pipefail
...
ENTRY="${ENTRY:-diffusion}"
CMD=(
    python -m "recipes.${ENTRY}"
